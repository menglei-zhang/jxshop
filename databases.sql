...
create table jx_lunbo(
  id int unsigned not null auto_increment comment '轮播ID',
  name varchar(100) not null default '' comment '轮播名称',
  type char(1) not null default '' comment '状态',
  isshow char(1) not null default '1' comment '是否显示',
  PRIMARY KEY ( `id` )
  );
  alter table jx_lunbo add column link varchar(200) not null default '' comment '链接';
